...
def new_disruption():
    ...
    for label_text, entry_key in info:
        ...
        entry = tk.Entry(new_disruption_window, font=("Helvetica", 16, "bold italic"), borderwidth=0, highlightthickness=0)
        entry.pack(pady=5)
...
def save_new_disruption(disruption_information):
    ...
    if disruption_information_keys["stm_oorz_groep"] in oorz_groep_mapping:
        disruption_information_keys["stm_oorz_groep"] = oorz_groep_mapping[disruption_information_keys["stm_oorz_groep"]]
    else:
        messagebox.showwarning("Ongeldige invoer", "Vul een geldige waarde in voor Oorzaak Groep: 'ONR-DERD', 'WEER', 'ONR-RIB' of 'TECHONV'")
        return
    ...
    disruption_information["status_betrouwbaarheid"] = None
    ...
    csv_data = {
        ...
        "stm_oorz_groep": disruption_information_keys["stm_oorz_groep"],
        ...
        "status_betrouwbaarheid": disruption_information["status_betrouwbaarheid"],
...
def update_csv_with_prediction(disruption_input, prediction_result, trustability):
    df = pd.read_csv("storing_Gegevens.csv", sep=";")
    disruption_criteria = (df["stm_sap_meld_ddt"][0] == disruption_input["stm_sap_meld_ddt"][0])
...
def run_base_model(disruption_input):
    ...
    #dit nog invullen met de betrouwbaarheid van de berekening
    trustability = None
    #ongebruikte kolommen nog verwijderen
    # stm_sap_meld_ddt    stm_geo_mld    stm_prioriteit    stm_aanngeb_dd    stm_oorz_groep    stm_oorz_code    stm_contractgeb_gst    stm_techn_gst    stm_progfh_in_duur    stm_fh_status
    print(input_df)
    ...
    update_csv_with_prediction(disruption_input, time_estimate, trustability)
